...
async def save_db_and_notify(tg_app: Application, db: Dict[str, Any], reason: str) -> None:
    save_db(db)
    try:
        ...
        logger.exception("DB notify failed: %s", e)
...
def ensure_user(db: Dict[str, Any], user) -> Dict[str, Any]:
    ...
    if user_id not in db["users"]:
        ...
        display_name = user.username or user.first_name or "کاربر"
        ...
        db["users"][user_id]["display_name"] = user.username or user.first_name or "کاربر"
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await save_db_and_notify(context.application, db, f"user_start:{user.id}")
...
async def my_link_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await save_db_and_notify(context.application, db, f"my_link:{user.id}")
...
async def reply_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await save_db_and_notify(context.application, db, f"reply_message:{msg_id}")
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_anonymous_message") and context.user_data.get("target_user_id"):
        ...
        await save_db_and_notify(context.application, db, f"new_message:{msg_id}")
        ...
        try:
            ...
            logger.exception("Anonymous message send failed: %s", e)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("reply:"):
        _, msg_id, sender_code = data.split(":", 2)
        ...
        context.user_data["reply_target_user_id"] = msg_data["sender_id"]
        context.user_data["reply_to_message_id"] = int(msg_id)

        await query.message.reply_text(f"پاسخ خودت به کاربر با کد {sender_code} رو بفرست.")
        return

    if data.startswith("react:"):
        _, msg_id, _ = data.split(":", 2)
...
telegram_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, reply_text_handler), group=0)
telegram_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, text_message_handler), group=1)
